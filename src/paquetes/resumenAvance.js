    ...
    SELECT
      ...
      COUNT(DISTINCT
        CASE WHEN a.id_cita IS NOT NULL
             THEN a.id_cita::text || '-' || a.id_correlativo::text
        END
      )::INT AS cantidad_realizada
...
async function getDashboard(anio) {
  ...
  let meta_pp0131   = totalCerrados > 0
  ...
  // 1. Agregar a la lista de "Paquetes" Mensuales
  let acpPaquete = paquetes.find(p => p.nombre_paquete.toLowerCase().includes('acompañamiento'));
  if (!acpPaquete) {
    acpPaquete = {
      id_paquete: 'ACP_001',
  ...
  for (const r of acpRows) {
    if (r.mes >= 1 && r.mes <= 12) {
      acpPaquete.meses[r.mes - 1] += r.cantidad;
      acpPaquete.total_acumulado += r.cantidad;
    }
  }

  // 2. Filtrar "Acompañamiento" de la Distribución para que no salga en los gráficos
  const distribucionFiltrada = distribucion.filter(d => !d.nombre.toLowerCase().includes('acompañamiento'));

  // 3. Opcional: Sumarlo a los "Totales" globales del Dashboard
  // Removido - ACP es actividad de conteo libre, no aporta a la meta oficial.
  ...
  return {
    ...
    vencidos,
    proximosAVencer,
    distribucion: distribucionFiltrada,
    paquetes,
    meta_pp0131,
